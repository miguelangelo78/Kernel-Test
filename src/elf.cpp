...
inline char * elf_str_table(elf32_ehdr * elf_header) {
	return elf_header->e_shstrndx == SHN_UNDEF ? 0 : (char*)elf_header + elf_section(elf_header, elf_header->e_shstrndx)->sh_offset;
}

static inline char *elf_lookup_string(elf32_ehdr *elf_header, elf32_shdr * elf_section) {
	char *strtab = elf_str_table(elf_header);
	return strtab ? strtab + elf_section->sh_name + 1 : 0;
}

static inline char *elf_lookup_string(elf32_ehdr *elf_header, elf32_sym * elf_symtable) {
	char *strtab = elf_str_table(elf_header);
	return strtab ? strtab + elf_symtable->st_name + 1 : 0;
}
...
static inline elf32_sym * elf_symtable(elf32_ehdr * header) {
	...
	return symsection ? (elf32_sym*)((uint32_t)header + symsection->sh_offset + 1) : 0;
}
...
static inline uintptr_t elf_get_symval(elf32_ehdr * header, elf32_shdr * symsection, int index) {
	...
	return (BASE_OFF(header) + symbol->st_value + elf_section(header, symbol->st_shndx)->sh_offset + 1);
}

static inline uintptr_t elf_get_symval(elf32_ehdr * header, elf32_shdr * symsection, char * symname, char delimiter) {
	for(int i = 0; i < elf_symtab_entry_count(symsection);i++) {
		...
		if(delimiter) {
			char * delim = strchr(symname_temp, (int)delimiter);
			if(delim) {
...
char * elf_parse(uint8_t * blob, int blobsize) {
	...
	Module::modent_t * mod = (Module::modent_t *)elf_get_symval(head, symsection, "modent_", '_');
	//mod->init();
	kprintf("symbol: %s", mod->name);
