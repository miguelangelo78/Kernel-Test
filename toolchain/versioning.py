import os, sys, re, getpass, socket, platform
...
if os.path.exists(version_src_file) and os.path.exists(version_header_file):
	...
	with open(version_src_file, 'r+') as ver_src:
		...
		if(builtby_match):
			whole_file = re.sub(r'ver_kernel_builtby(?:.+?)?=.+?;','ver_kernel_builtby = (char*)"'+getpass.getuser()+'@'+socket.gethostname()+ ' plat: ' + platform.system() + '";', whole_file)
